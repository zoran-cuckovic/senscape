...
class NetworkVector(GeoAlgorithm):
...
##                self.TYPE, 5))
    ...
    def processAlgorithm(self, progress):
        ...
        print "mins"
        sinks = nt.sinks(x_parent, y_parent)

        print "mins"
        ranked = nt.ranks(ids, r, mask = sinks==0 )

        print "accumulation"
        mins = nt.accum_min (ranked,  x_parent, y_parent, steps)

        print 'assembly'
        t=tg.TopoGraph()
        t.assembly(ids, x_parent, y_parent, steps, mins)

        print 'shapefile'
        prj= d.GetProjection(); transf = d.GetGeoTransform()
        t.write_shp(Output_vector, prj, transf )        
